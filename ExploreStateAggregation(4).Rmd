...
#Estimate everything has even level
state2var <- function(level,value){
  # (((i-1) + level*(j-1) + level*level*(k-1) +level*level*level*(l-1))+1)
  i = value %% level
  value = value %/% level
  j = (value + 1) %% level
  value = value %/% level
...
summary(rowSums(T0))
T0xPOS = matrix(0,totalstate/Splitstate,totalstate/Splitstate)
T1xPOS = matrix(0,totalstate/Splitstate,totalstate/Splitstate)
```

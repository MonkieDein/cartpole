...
output: pdf_document
---
20 STATES FOR EACH VARIABLES (no position variable)
...
CartVelQuantile  = quantile(df$CartVelocity,(0:20)*0.05)
# Discretize state as 10 differnt quantile for eqch variable
PoleAngleQuantile  = quantile(df$PoleAngle,(0:20)*0.05)
# Discretize state as 10 differnt quantile for eqch variable
PoleVelocityQuantile  = quantile(df$PoleVelocity,(0:20)*0.05)
...
# create penalty
df2$Reward[which(df2$Reward==0)] = -500 # penalty -50
# NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW NEW
n = length(df2$Step)
...
```{r}
StateSpace = rep(0,car_vel_len^3)
...
summary(v)
summary(result)
dirname = "Mdp20stateNOPOS"
if (!dir.exists(dirname)){
  dir.create(dirname)
}
k=paste0(dirname,"/all4")
if (TRUE){
